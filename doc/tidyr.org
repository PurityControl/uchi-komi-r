...
The next stage involve making the values in the test column variables that have
the grade as their value.
...
Finally we want to remove the class prefixe from the values in the class
